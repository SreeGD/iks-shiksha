...
DECKS="$ROOT/.build-tmp/darshana-decks"
...
# The Marp-style decks are generated by scripts/build-darshana.py into
# .build-tmp/darshana-decks/. Regenerate if missing.
if [ ! -d "$DECKS" ] || [ -z "$(ls -A "$DECKS" 2>/dev/null)" ]; then
  echo "Deck files missing — running build-darshana.py first..."
  python3 "$ROOT/scripts/build-darshana.py"
fi
...
build_one() {
  ...
  # PPTX: route the Marp deck through generate-pptx.js (band-coloured, html2pptx),
  # the same pipeline the per-day decks use. IKS_LENIENT skips any overflow slide.
  local deck="$DECKS/${mod}/${band}.md"
  if [ -f "$deck" ]; then
    IKS_LENIENT=1 node "$ROOT/scripts/generate-pptx.js" \
      --in "$deck" \
      --out "$OUT_PPTX/${stem}.pptx" \
